...
class TrainingConfig:
    ...
    caption_prompt: str = "Write a detailed description for this image in 50 words or less. Do NOT mention any text that is in the image."
    ...
    def _validate(self):
        ...
        if not self.image_urls and not self.video_urls:
            raise ValueError("At least one of image_urls or video_urls must be provided")
        ...
        if self.caption_mode in ["videos", "both"] and not self.gemini_api_key:
            raise ValueError(
                "Video captioning requires gemini_api_key"
            )

        # Validate URLs format
        for url in self.image_urls + self.video_urls:
        ...
        logger.info(f"Configuration validated: model={self.model_type}, "
    ...
    def _merge_defaults(self):
        ...
        logger.info(f"Training params: epochs={self.training_params['epochs']}, "
